...
if (!empty($empid)) {
    ...
    $fileurl = 'worklist/' . $empid . 'WORK_LIST_EMP.json';
    ...
    $file_wk = fopen('worklist/'.$empid.'WORK_LIST_EMP.json', 'w');
...
function GetARCollection($empid) {
	...
	$soap_request = '<?xml version="1.0" encoding="utf-8"?>
				<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
				  <soap:Body>
					<GET_WORKLIST_GROUP_AUTODIAL xmlns="http://tempuri.org/">
					  <USER>'.$empid.'</USER>
					</GET_WORKLIST_GROUP_AUTODIAL>
				  </soap:Body>
				</soap:Envelope>';
    ...
    $header = array(
        ...
        "Host:" . $websrvnoport,
        "Pragma: no-cache",
        "SOAPAction: \"http://tempuri.org/GET_WORKLIST_GROUP_AUTODIAL\"",
        "Content-length: " . strlen($soap_request),
    );

    $soap_do = curl_init();
    curl_setopt($soap_do, CURLOPT_URL, $websrv . '/WebService_Main_AR_Collection.asmx?op=GET_WORKLIST_GROUP_AUTODIAL');
    ...
    $response = $content->children('http://schemas.xmlsoap.org/soap/envelope/')->Body->children()->GET_WORKLIST_GROUP_AUTODIALResponse->GET_WORKLIST_GROUP_AUTODIALResult;
