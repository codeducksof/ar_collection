...
if (!empty($empid)) {
    ...
    $fileurl = 'worklist/' . $empid . 'WORK_EMP.json';
    ...
    $file_wk = fopen('worklist/'.$empid.'WORK_EMP.json', 'w');
...
function GetARCollection($empid) {
	...
	$soap_request = '<?xml version="1.0" encoding="utf-8"?>
				<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
				  <soap:Body>
					<LOAD_ARM_T_GROP_CALL xmlns="http://tempuri.org/" />
				  </soap:Body>
				</soap:Envelope>';
    ...
    $header = array(
        ...
        "Host:" . $websrvnoport,
        "Pragma: no-cache",
        "SOAPAction: \"http://tempuri.org/LOAD_ARM_T_GROP_CALL\"",
        "Content-length: " . strlen($soap_request),
    );

    $soap_do = curl_init();
    curl_setopt($soap_do, CURLOPT_URL, $websrv . '/WebService_Main_AR_Collection.asmx?op=LOAD_ARM_T_GROP_CALL');
    ...
    $response = $content->children('http://schemas.xmlsoap.org/soap/envelope/')->Body->children()->LOAD_ARM_T_GROP_CALLResponse->LOAD_ARM_T_GROP_CALLResult;
