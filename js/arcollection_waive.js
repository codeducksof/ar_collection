...
$(function () {
    ...
    $.ajax({
        url:'worklist/' + $("#txtEmpID").val() + 'WAIVE.json',
            type:'HEAD',
            error: function()
            {            
            ...
            success: function()
            {  
                LoadDatatable('worklist/'+$("#txtEmpID").val() + 'WAIVE.json');
            }
    });
...
function LoadDataJson(){
    ...
    $.ajax('get_arcollection_waive_level.php', {       
        method: 'GET',
        data: {wktype: "WAIVE"}
       ...
       table.destroy();  
       var file = 'worklist/' + $("#txtEmpID").val() + 'WAIVE.json';
       LoadDatatable(file);        
...
var table = $('#WORKLISTOD').DataTable();
    ...
    table.columns(6).header().to$().text('วันที่บันทึก');
    table.columns(7).header().to$().text('ผลการโทร');   
    ...
    $('#WORKLISTOD tbody').on('click', 'tr', function () {
            ...
            var tbRow = table.row(this).data();
            //console.log(tbRow.ARM_ACC_NO);
            window.open("arcollection_detial_waive.php?pageslug=REQ&readonly=true&accno=" + btoa(tbRow.ARM_ACC_NO),'_blank');
