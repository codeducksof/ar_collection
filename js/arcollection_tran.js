...
$(function () {
    ...
    $.ajax({
        url:'worklist/' + $("#txtEmpID").val() + 'TRAN.json',
            type:'HEAD',
            error: function()
            {            
            ...
            success: function()
            {  
                LoadDatatable('worklist/'+$("#txtEmpID").val() + 'TRAN.json');
            }
    });
...
function LoadDataJson(){
    ...
    $.ajax('get_arcollection.php', {       
       ...
       table.destroy();  
       var file = 'worklist/' + $("#txtEmpID").val() + 'TRAN.json';
       LoadDatatable(file);        
...
var table = $('#WORKLISTOD').DataTable();
    ...
    $('#WORKLISTOD tbody').on('click', 'tr', function () {
            ...
            var tbRow = table.row(this).data();
            //console.log(tbRow.ARM_ACC_NO);
            window.open("arcollection_detial.php?pageslug=TRAN&accno=" + btoa(tbRow.ARM_ACC_NO),'_blank');
