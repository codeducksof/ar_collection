...
(function ($) {
    $.fn.EasyTree = function (options) {
        var defaults = {
            ...
            addable: false,
            i18n: {
                deleteNull: 'เลือกรายการที่ต้องการลบ',
                deleteConfirmation: 'คุณต้องการลบใช่ หรือ ไม่ ?',
                confirmButtonLabel: 'ยืนยัน',
                editNull: 'เลือกรายการที่ต้องการแกไข',
                editMultiple: 'Only one node can be edited at one time',
                addMultiple: 'Select a node to add a new node',
                collapseTip: 'ย่อ',
                expandTip: 'ขยาย',
                selectTip: 'เลือก',
                unselectTip: 'ยกเลิกการเลือก',
                editTip: 'แก้ไข',
                addTip: 'เพิ่ม',
                deleteTip: 'ลบ',
        ...
        var createInput = $('<div class="input-group"><input type="text" class="form-control"><span class="input-group-btn"><button id="btnAdd" type="button" class="btn btn-default btn-success confirm"></button> </span><span class="input-group-btn"><button type="button" class="btn btn-default cancel"></button> </span> </div> ');
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var ParentID = $(selected).attr('group');
                        var GroupName = $(createInput).find('input').val();                        
                        var GroupID = InserGroup(GroupName,ParentID);
                        //console.log(GroupID);
                        
                        var item = $('<li parent="' + ParentID + '" group="' + GroupID + '"><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        ...
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {
                                if ($(editor).val() !== '') {
                                    ...
                                    var ParentID = $(selected).attr('parent');
                                    var GroupName = editor.val();                        
                                    var GroupID = $(selected).attr('group');
                                    UpDateGroup(GroupID,GroupName,ParentID);
                ...
                $(easyTree).find('.easy-tree-toolbar .remove > button').attr('title', options.i18n.deleteTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content').html(options.i18n.deleteConfirmation)
                            .append('<a id="btnDelete" style="margin-left: 10px;" class="btn btn-default btn-danger confirm"></a>')
...
function InserGroup(groupName, Parent) {
    
    var result = "";
    $.ajax('manage_goup_level.php', {
        method: 'GET',
        async: false,
        data: {"g_id": "-", "g_name": groupName, "parent": Parent, "type": "I"},
        dataType: 'json'
...
function DelGroup(groupID) {
    ...
    $.ajax('manage_goup_level.php', {
        method: 'GET',
        async: false,
        data: {"g_id": groupID, "g_name": "-", "parent": "-", "type": "D"},
        dataType: 'json'
    }).then(function success(data) {
...
function UpDateGroup(groupID,groupName,Parent) {
    
    var result = "";
    $.ajax('manage_goup_level.php', {
        method: 'GET',
        async: false,
        data: {"g_id": groupID, "g_name": groupName, "parent": Parent, "type": "U"},
        dataType: 'json'
    }).then(function success(data) {
