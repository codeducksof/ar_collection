...
$(function () {
    ...
    $.ajax({
        url:'worklist/' + $("#txtEmpID").val() + 'PTP.json',
            type:'HEAD',
            error: function()
            {            
            ...
            success: function()
            {  
                LoadDatatable('worklist/'+$("#txtEmpID").val() + 'PTP.json');
            }
    });
...
function LoadDataJson(){
    ...
    $.ajax('get_arcollection.php', {       
       ...
       table.destroy();  
       var file = 'worklist/' + $("#txtEmpID").val() + 'PTP.json';
       LoadDatatable(file);      
...
var table = $('#WORKLISTOD').DataTable();
    ...
    $('#WORKLISTOD tbody').on('click', 'tr', function () {
            ...
            var tbRow = table.row(this).data();
            //console.log(tbRow.ARM_ACC_NO);
            window.open("arcollection_detial.php?pageslug=PTP&accno=" + btoa(tbRow.ARM_ACC_NO),'_blank');
            //window.location.assign("arcollection_detial.php?accno=" + btoa(tbRow.ARM_ACC_NO));        
        });
