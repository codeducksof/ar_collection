...
$(function () {
    ...
    $.ajax({
        url:'worklist/' + $("#txtEmpID").val() + 'END.json',
            type:'HEAD',
            error: function()
            {            
            ...
            success: function()
            {  
                LoadDatatable('worklist/'+$("#txtEmpID").val() + 'END.json');
            }
    });
...
function LoadDataJson(){
    ...
    $.ajax('get_arcollection.php', {       
       ...
       table.destroy();  
       var file = 'worklist/' + $("#txtEmpID").val() + 'END.json';
       LoadDatatable(file);        
...
var table = $('#WORKLISTOD').DataTable();
    ...
    $('#WORKLISTOD tbody').on('click', 'tr', function () {
            ...
            var tbRow = table.row(this).data();
            //console.log(tbRow.ARM_ACC_NO);
            window.open("arcollection_detial.php?pageslug=END&accno=" + btoa(tbRow.ARM_ACC_NO),'_blank');
