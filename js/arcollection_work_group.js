...
$(function () {
    ...
    $('#frmImportFile').validator().on('submit', function (e) {
        ...
        if (e.isDefaultPrevented(e)) {
            ...
            $.ajax('import_arcollection_all_group.php', {       
                method: 'POST',
                contentType: false,
                cache: false,
                processData: false,
...
function ExportFile(){
    ...
    $.ajax('export_arcollection_all_group.php', {       
        method: 'GET',
        dataType: 'json',
    }).then(function success(data) {  
    var result = data.data;  
...
function LoadDataJson(){
    ...
    $.ajax('get_arcollection_work_group.php', {       
        method: 'POST',
        async: true,
        cache: false,
        timeout: 60000,
       ...
       //console.log(data);
        ...
        if(result == 'DEADLOCK'){
            ...
            table.destroy();
            var file = 'worklist/' + $("#txtEmpID").val() + 'GROUP_WORK.json';
            LoadDatatable(file);
        }
