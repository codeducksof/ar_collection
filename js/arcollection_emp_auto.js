...
$(function () {
    ...
    $('#frmImportFile').validator().on('submit', function (e) {
        ...
        if (e.isDefaultPrevented(e)) {
            ...
            $.ajax('import_arcollection_emp_all.php', {       
                method: 'POST',
                contentType: false,
                cache: false,
                processData: false,
...
function ExportFile(){
    ...
    $.ajax('export_arcollection_all_emp_auto.php', {       
        method: 'GET',
        dataType: 'json',
    }).then(function success(data) {  
    var result = data.data;  
...
function LoadDataJson(){
    ...
    $.ajax('get_arcollection_work_emp.php', {       
        method: 'POST',
        async: true,
        cache: false,
        //timeout: 60000,
       ...
       //console.log(data);
        ...
        if(result == 'DEADLOCK'){
           ...
           alert('ขณะนี้การใช้งานโปรแกรม มีใช้งานข้อมูลค่อนข้างเยอะ โปรดลองใหม่ภายหลัง ขออภัยในความไม่สะดวก');
            ...
            table.destroy();
            var file = 'worklist/' + $("#txtEmpID").val() + 'WORK_EMP.json';
            LoadDatatable(file);
        }
...
function LoadDatatable(file){
    ...
    var table = $('#WORKALL').dataTable({
        ...
        columns: [
            ...
            {"data": "ARM_GROP_CHILD", "className": "text-center", "defaultContent": ''}, 
			{"data": "ARM_GRUOP_PARENT", "className": "text-center", "defaultContent": ''},   			
