...
function ImportFile($fiePathUrl,$empid,$fileName){
    ...
    $soap_request = '<?xml version="1.0" encoding="utf-8"?>
                    <soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
                      <soap:Body>
                        <Import_File_Excel_ARM_T_GROP_CALL  xmlns="http://tempuri.org/">
                          <EMP_CODE>'.$empid.'</EMP_CODE>
                          <File_Name>'.$fileName.'</File_Name>
                          <Full_Path>'.$fiePathUrl.'</Full_Path>
                        </Import_File_Excel_ARM_T_GROP_CALL >
                      </soap:Body>
                    </soap:Envelope>';
    ...
    $header = array(
        ...
        "Host:" . $websrvnoport,
        "Pragma: no-cache",
        "SOAPAction: \"http://tempuri.org/Import_File_Excel_ARM_T_GROP_CALL\"",
        "Content-length: " . strlen($soap_request),
    );

    $soap_do = curl_init();
    curl_setopt($soap_do, CURLOPT_URL, $websrv . '/WebService_Main_AR_Collection.asmx?op=Import_File_Excel_ARM_T_GROP_CALL');
    ...
    $response = $content->children('http://schemas.xmlsoap.org/soap/envelope/')->Body->children()->Import_File_Excel_ARM_T_GROP_CALLResponse->Import_File_Excel_ARM_T_GROP_CALLResult[0];
