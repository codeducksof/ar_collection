...
if (!empty($empid)) {
    ...
    $fileurl = 'worklist/' . $empid . 'WORK.json';
    ...
    $file_wk = fopen('worklist/'.$empid.'WORK.json', 'w');
...
function GetARCollection($empid) {
    ...
    $soap_request = '<?xml version="1.0" encoding="utf-8"?>
                    <soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
                      <soap:Body>
                        <GET_For_Export_Excel xmlns="http://tempuri.org/">
                          <ACC_TYPE>ACTIVE</ACC_TYPE>
                        </GET_For_Export_Excel>
                      </soap:Body>
                    </soap:Envelope>';
    ...
    $header = array(
        ...
        "Host:" . $websrvnoport,
        "Pragma: no-cache",
        "SOAPAction: \"http://tempuri.org/GET_For_Export_Excel\"",
        "Content-length: " . strlen($soap_request),
    );

    $soap_do = curl_init();
    curl_setopt($soap_do, CURLOPT_URL, $websrv . '/WebService_Main_AR_Collection.asmx?op=GET_For_Export_Excel');
    ...
    $response = $content->children('http://schemas.xmlsoap.org/soap/envelope/')->Body->children()->GET_For_Export_ExcelResponse->GET_For_Export_ExcelResult[0];
